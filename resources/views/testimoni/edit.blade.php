			  	...
			  	<form action="{{ route('testimoni.update',$testimonis->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                    	<label class="cc-payment" class="control-label mb-1">Gambar</label>
			  			...
			  			<p>
			  				<br>
			  				<img src="{{ asset('assets/img/gambar/' .$testimonis->gambar)}}" style="max-height: 125px; max-width: 125px; margin-top: 7px;" alt="">
			  			</p>
			  			@endif
			  			<input type="file" name="gambar" value="{{ $testimonis->gambar }}">
                    </div>

			  		<div class="form-group {{ $errors->has('alamat') ? ' has-error' : '' }}">
			  			<label class="control-label">Alamat</label>	
			  			<input type="text" name="alamat" class="form-control" value="{{ $testimonis->alamat }}"  required>
			  		...
			  		<div class="form-group {{ $errors->has('content') ? ' has-error' : '' }}">
			  			...
			  			<textarea id="text" type="ckeditor" name="content" class="ckeditor" required>{{ $testimonis->content }}</textarea>
